(function () {
  ...
  var customPool = [];
  var lives = 3;
  var gameStyle = 'single';
  var wordMin = 1;
  var wordMax = 6;
  var SETTINGS_KEY = 'kanaShoot_v2_settings';
  ...
  function saveSettings() {
    try {
      var o = {
        ...
        gamestyle: document.getElementById('opt-gamestyle').value,
  ...
  function loadSettings() {
    try {
      ...
      var styleSel = document.getElementById('opt-gamestyle');
      if (o.gamestyle) styleSel.value = o.gamestyle;
      var wMin = document.getElementById('opt-word-min');
      var wMax = document.getElementById('opt-word-max');
      if (o.wordMin != null) wMin.value = Math.max(2, Math.min(6, parseInt(o.wordMin, 10) || 2));
      if (o.wordMax != null) wMax.value = Math.max(2, Math.min(6, parseInt(o.wordMax, 10) || 5));
      var wrap = document.getElementById('word-length-wrap');
      wrap.classList.toggle('hidden', styleSel.value !== 'words');
  ...
  function spawn() {
    ...
    if (gameStyle === 'words') {
      ...
      symbols.push({ el: el, romaji: item.romaji, top: 0 });
    } else {
      ...
      symbolsEl.appendChild(el);
      symbols.push({ el: el, romaji: single.romaji, top: 0 });
    }
  }
  ...
  function startGame() {
    ...
    wordMin = Math.max(2, Math.min(6, parseInt(wMinIn.value, 10) || 2));
    wordMax = Math.max(2, Math.min(6, parseInt(wMaxIn.value, 10) || 5));
  ...
  function init() {
    ...
    document.getElementById('opt-gamestyle').addEventListener('change', function () {
      var wrap = document.getElementById('word-length-wrap');
      wrap.classList.toggle('hidden', this.value !== 'words');
      saveSettings();
    });
